  ...
  public static final EasyFlow<BattleContext> FLOW =
    from(PLAYER_TURN).transit(
      ...
      on(FRIENDLY_UNIT_SELECTED).to(SELECTING_UNIT_ACTION).transit(
        ...
        on(INFO_ACTION_SELECTED).to(SHOWING_FRIENDLY_DETAILS).transit(
          on(UNIT_DETAILS_CLOSED).to(SELECTING_UNIT_ACTION)
        ),
        on(FRIENDLY_UNIT_STATS_SELECTED).to(SHOWING_FRIENDLY_DETAILS),
      ...
      on(ENEMY_UNIT_SELECTED).to(SHOWING_ENEMY_DETAILS).transit(
